...
void LCD_ShowChar16x32(u16 x,u16 y,u8 index, u16 color)
{
	...
	u8 *temp,size1;
	u8 x_size,y_size;
	temp=char_16x32;	//选择字号

	x_size = 16;
	y_size = 32;
	LCD_Address_Set(x,y,x+x_size-1,y+y_size-1);		//设置一个汉字的区域
	size1=x_size*y_size/8;							//一个汉字所占的字节
	temp+=index*size1;								//写入的起始位置
	for(j=0;j<size1;j++)
	{
		for(i=0;i<8;i++)
...
void LCD_ShowFlexibleChinese(u16 x,u16 y,u8 index,u8 sizex,u8 sizey,u16 color)
{
	...
	//if(sizex==xx&&sizey==xx)temp=Hzkxx;//用户自定义汉字大小
...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 mode,u16 color)
{
	...
	if(x>LCD_W-8||y>LCD_H-8)return;	    //设置窗口
	num=num-' ';//得到偏移后的值
